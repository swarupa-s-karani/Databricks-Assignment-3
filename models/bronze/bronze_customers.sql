...
{{ config(
    ...
    post_hook="INSERT INTO insurance_analytics.04_logs.dbt_logs 
               ...
               SELECT 
                   'bronze_customers' as dataset,
                   current_timestamp() as time_processed,
                   (SELECT COUNT(*) FROM delta.`/Volumes/insurance_analytics/00_landing/streaming/customers/`) as source_records,
...
FROM delta.`/Volumes/insurance_analytics/00_landing/streaming/customers/`