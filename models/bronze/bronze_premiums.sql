--models/bronze/bronze_premiums.sql
{{ config(
    materialized='table',
    schema='01_bronze',
    post_hook="INSERT INTO insurance_analytics.04_logs.dbt_logs 
               ...
               SELECT 
                   'bronze_premiums' as dataset,
                   current_timestamp() as time_processed,
                   (SELECT COUNT(*) FROM delta.`/Volumes/insurance_analytics/00_landing/streaming/premiums/`) as source_records,
                   (SELECT COUNT(*) FROM {{ this }}) as target_records,
                   0 as bad_records,
                   'bronze_premiums' as model_name,
...
FROM delta.`/Volumes/insurance_analytics/00_landing/streaming/premiums/`